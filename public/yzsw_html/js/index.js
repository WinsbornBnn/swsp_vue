layui.use(['layer', 'dropdown'], () => {
  ...
  const getindexList = () => {
    ...
    if (fiveimages.records) {
      fiveimages.records.forEach((item) => {
        allimages.push(item.pictureone)
      })
      // 原来就有图片 >=1
      for (let i = 0; i < 5; i++) {
        if (allimages.length < 5) {
          allimages.push("./icons/nothing.png");
        }
        allimagesUrl.push(allimages[i].includes("temp") ? BaseUrl + "sys/common/static/" + allimages[i] : allimages[i]);
      }
    } else {
      // 原来就没有图片 0
      for (let i = 0; i < 5; i++) {
        allimages.push("./icons/nothing.png");
        allimagesUrl.push(allimages[i]);
      }
    }
    $("#left").html(`
    <div class="top">
