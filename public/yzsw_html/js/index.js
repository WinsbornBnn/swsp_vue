layui.use(['layer', 'dropdown','carousel'], () => {
  ...
  const getindexList = () => {
    ...
    if (fiveimages.records) {
      ...
      let yz_img = [];
      let yx_img = [];
      console.log(fiveimages.records);
    ...
    // $("#left").html(`
    // <div class="top">
    //       <div class="before u-f-ajc">
    //       <p>${data[0].value}</p>
    //       </div>
    //     </div>
    //     <div class="center" id="center">
    //       <img src="${allimagesUrl[0]}" alt>
    //     </div>
    //     <div class="bottom" id="center">
    //       <img src="${allimagesUrl[1]}" alt>
    //     </div>
    // `)
    // $("#right").html(` 
    // <div><img src="${allimagesUrl[2]}" alt></div>
    // <div><img src="${allimagesUrl[3]}" alt></div>
    // <div><img src="${allimagesUrl[4]}" alt></div>
    // `)
  ...
  /*map.on('click', (evt) => {
    map.forEachFeatureAtPixel(evt.pixel, (e) => {
      ...
      if (data.name === 'icon') {
        ...
        if (data.images) {
          // 原来就有图片 >=1
          newimages = data.images.split(",");
          for (let i = 0; i < 5; i++) {
            if (newimages.length < 5) {
              newimages.push("./icons/nothing.png");
            }
            imagesurl.push(newimages[i].includes("images") ? BaseUrl + "sys/common/static/" + newimages[i] : newimages[i]);
          }
        } else {
          // 原来就没有图片 0
          for (let i = 0; i < 5; i++) {
            newimages.push("./icons/nothing.png");
            imagesurl.push(newimages[i]);
          }
        }
        switch (data.design) {
          case '小区':
            ...
            <div class="after">
              ...
              <div class="show u-f">
                  ...
                  <ul class="right">
                    <p>${data.plot.zhrs} 人</p>
                    <p>${data.plot.zhysl} 万m³</p>
                    <p>${data.plot.rjys} L/(人·d) (定额值：${data.plot.dez})</p>
                    <p>${data.plot.xmmc}</p>
                    <p>${data.plot.jsjgtr}万元(其中：政策扶持${data.plot.cztr}万元)</p>
                  </ul>
              </div>
            </div>
          </div>
          ...
          case '企业':
            ...
            <div class="after">
              ...
              <div class="show u-f">
                  ...
                  <ul class="right">
                    <p>${data.company.hyfl}</p>
                    <p id="second">${data.company.cflyl}%</p>
                    <p>${data.company.xmmc}</p>
                    <p>${data.company.jsjgtr}万元(其中：政策扶持${data.company.cztr}万元)</p>
                  </ul>
              </div>
            </div>
          </div>
          ...
          <div class="bottom" id="bottom">
            ...
            for (let i = 0; i < zycp.length; i++) {
              ...
              $("#second").before(`
              <p>${zycp[i]}</p>
              <p>${data.company.sjysl} 万m³</p>
              <p>${data.company.dwcpys}${data.company.dwcpysdw} (定额值：${data.company.dez})</p>
              `);
            }
            break;
          case '灌区':
            ...
            <div class="after">
              ...
              <div class="show u-f">
                  ...
                  <ul class="right">
                    <p>${data.tank.gqgmlx}</p>
                    <p>${data.tank.zyzw}</p>
                    <p>${data.tank.sjysl} 万/m³</p>
                    <p>${data.tank.dwlzss} ㎡/亩</p>
                    <p>xxxx</p>
                    <p>${data.tank.xmmc}</p>
                    <p>${data.tank.jsjgtr}万元(其中：政策扶持${data.tank.cztr}万元)</p>
                  </ul>
              </div>
            </div>
          </div>
            ...
            leftElement.innerHTML = `
              ...
              <div class="after">
                ...
                <div class="show u-f">
                    ...
                    <ul class="right">
                      <p>${data.units.dwlx}</p>
                      <p>${data.units.ysrs} ${data.units.ysrsdw}</p>
                      <p>${data.units.sjysl} 万m³</p>
                      <p>${data.units.rjysl} ${data.units.ysldw}(定额值：${data.units.dez})</p>
                      <p>${data.units.xmmc}</p>
                      <p>${data.units.jsjgtr}万元(其中：政策扶持${data.units.cztr}万元)</p>
                    </ul>
                </div>
              </div>
            </div>
        ...
        $("#center").html(`<img src="${imagesurl[0]}" alt>`);
        $("#bottom").html(`<img src="${imagesurl[1]}" alt>`);
        rightElement.innerHTML = `
        <div><img src="${imagesurl[2]}" alt></div>
        <div><img src="${imagesurl[3]}" alt></div>
        <div><img src="${imagesurl[4]}" alt></div>
        `;
        content.innerHTML = `
             <div class="information">
              ${data.title}
...
function show_info(data){
  ...
  switch (data.design) {
    ...
    <div class="bottom" id="bottom">
      ...
      let zycp_new = data.company.zycp.split("、")
      for (let i = 0; i < zycp_new.length; i++) {
          $("#first").before(`
          <div class="item_div_text">主要产品${i + 1}:</div>
          <div class="item_div_text">${zycp_new_two[i]}</div>
          <div class="item_div_text">产品${i + 1}取水用水量:</div>
          <div class="item_div_text">${data.company.sjysl} 万m³</div>
          <div class="item_div_text">产品${i + 1}用水量:</div>
          <div class="item_div_text">${data.company.dwcpys}${data.company.dwcpysdw} (定额值：${data.company.dez})</div>
          `);
      }
      break;
    ...
    case '企业':
      ...
      <div class="bottom" id="bottom">
        ...
        let zycp_new_two = data.company.zycp.split("、")
        for (let i = 0; i < zycp_new_two.length; i++) {
          $("#first").before(`
          <div class="item_div_text">主要产品${i + 1}:</div>
          <div class="item_div_text">${zycp_new_two[i]}</div>
          <div class="item_div_text">产品${i + 1}取水用水量:</div>
      ...
      leftElement.innerHTML = `
        ...
        <div class="after">
          ...
          <marquee direction="up" onMouseOver="this.stop()" onMouseOut="this.start()"  scrolldelay="200" style="    width: 100%;height: 100%;margin-top: 0.36rem;margin-left: 0.25rem;" >
              ...
              <div class="item_div_text">2019年实际用水量:</div>
        ...
        <div class="after">
          ...
          <div class="show u-f">
              ...
              <ul class="right">
                <p>${data.plot.hyfl}</p>
                <p>${data.plot.zycp}</p>
                <p>${data.plot.zycpclz}（${data.plot.zycpcldw}）</p>
                <p>${data.plot.sjysl}</p>
                <p>${data.plot.cfysl}</p>
                <p>${data.plot.cflyl}</p>
                <p>${data.plot.xmmc}</p>
                <p>${data.plot.jsjgtr}(其中财政投入${data.plot.cztr})</p>
              </ul>
          </div>
        </div>
      </div>
...
function show_img(data){
  ...
  if (obj_data.images) {
    ...
    newimages = data.images.split(",");
    for (let i = 1; i < 6; i++) {
      $("#ba_img0"+i).HTML('<img src="icons/11.png" class="banner_img" />');
    }
